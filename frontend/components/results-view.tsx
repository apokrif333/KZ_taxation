...
export function ResultsView({ result, brokers, onRestart }: ResultsViewProps) {
  ...
  return <section className="flex flex-col gap-8 py-8" aria-labelledby="result-title">
    <header className="flex flex-col gap-4 border-b pb-6 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-4"><span className="flex size-11 items-center justify-center rounded-full bg-primary/10 text-primary"><CheckCircle2 aria-hidden="true" /></span><div><p className="text-sm font-medium text-primary">РАСЧЁТ ЗАВЕРШЁН</p><h1 id="result-title" className="text-2xl font-semibold tracking-tight">Файлы готовы</h1><p className="mt-1 text-sm text-muted-foreground">Налоговый год: {result.tax_year}. Скачайте audit-файлы и черновик формы 270.00.</p><div className="mt-3 grid max-w-4xl gap-3 md:grid-cols-2"><Link href="/faq/audit-file" className="flex items-start gap-2 rounded-md border border-primary/15 bg-accent/25 px-3 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-accent/50 hover:text-foreground"><FileSearch className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden="true" /><span><span className="inline-flex items-center gap-1">Описание audit XLSX-файлов и их структуры. <span className="font-medium text-primary">Что находится в audit-файле <ArrowRight className="inline-block size-3.5" aria-hidden="true" /></span></span><span className="mt-1 block text-xs leading-relaxed text-primary/80">Нажав на эту ссылку, текущий расчёт будет удалён, и вы не сможете вернуться на эту страницу.</span></span></Link><Link href="/faq/form270-upload" className="flex items-start gap-2 rounded-md border border-primary/15 bg-accent/25 px-3 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-accent/50 hover:text-foreground"><FileJson className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden="true" /><span><span className="font-medium text-primary">Загрузка JSON формы 270.00 и оплата налога <ArrowRight className="inline-block size-3.5" aria-hidden="true" /></span><span className="mt-1 block text-xs leading-relaxed">Инструкция по проверке данных КГД, загрузке JSON, отправке декларации и оплате ИПН.</span><span className="mt-1 block text-xs leading-relaxed text-primary/80">Нажав на эту ссылку, текущий расчёт будет удалён, и вы не сможете вернуться на эту страницу.</span></span></Link></div></div></div><Button variant="outline" onClick={onRestart}><RotateCcw data-icon="inline-start" />Начать новый расчёт</Button></header>
