  ...
  return (
    <section aria-labelledby="calculation-title" className="grid gap-6 lg:grid-cols-[1fr_19rem]">
      <Card className="border-border/80 shadow-sm">
        ...
        <CardContent className="flex flex-col gap-5">
          ...
          {brokers.map((broker) => broker.account_id_mode === 'auto'
            ? <BrokerReportCard key={broker.code} broker={broker} reports={autoFiles[broker.code] || []} onFiles={(files) => onAddAutoFiles(broker, files)} onRemove={(reportId) => onRemoveAutoFile(broker.code, reportId)} />
            : <ManualBrokerReportCard key={broker.code} broker={broker} groups={manualGroups.filter((group) => group.broker === broker.code)} onAddGroup={() => onAddManualGroup(broker.code)} onRemoveGroup={onRemoveManualGroup} onAccountChange={onManualAccountChange} onFiles={onAddManualFiles} onRemoveFile={onRemoveManualFile} />,
          )}
      ...
      <aside><Card className="sticky top-6 border-border bg-card shadow-md shadow-primary/5">
        ...
        <CardContent className="border-t py-5"><label className="flex cursor-pointer items-start gap-3 rounded-md border border-primary/15 p-3 text-sm"><Checkbox checked={form27005} onCheckedChange={(value) => onForm27005Change(Boolean(value))} aria-label="Я владелец бизнеса или попадаю под закон о противодействии коррупции" /><span className="leading-snug">Я владелец бизнеса или попадаю под закон о противодействии коррупции</span></label></CardContent>
...
function BrokerReportCard({ broker, reports, onFiles, onRemove }: { broker: BrokerConfig; reports: SelectedReport[]; onFiles: (files: File[]) => void; onRemove: (reportId: string) => void }) {
  ...
  return <div className="rounded-lg border bg-card p-4"><BrokerTitle broker={broker} /><p className="mt-1 text-sm text-muted-foreground">{reports.length ? `${reports.length} ${pluralFiles(reports.length)}` : 'Файлы не добавлены'}</p><FilePicker className="mt-3 w-full" broker={broker} onFiles={onFiles} /><ReportList reports={reports} onRemove={onRemove} />{guide && <div className="mt-4 flex flex-wrap items-center gap-x-2 gap-y-1 border-t border-primary/10 pt-3 text-sm"><BookOpen className="size-4 text-primary" aria-hidden="true" /><span className="font-medium">Инструкция:</span><Link href={guide.href} className="text-primary underline-offset-4 hover:underline">{guide.label}</Link></div>}</div>
}

function ManualBrokerReportCard({ broker, groups, onAddGroup, onRemoveGroup, onAccountChange, onFiles, onRemoveFile }: { broker: BrokerConfig; groups: ManualAccountGroup[]; onAddGroup: () => void; onRemoveGroup: (groupId: string) => void; onAccountChange: (groupId: string, value: string) => void; onFiles: (groupId: string, broker: BrokerConfig, files: File[]) => void; onRemoveFile: (groupId: string, reportId: string) => void }) {
  return <div className="rounded-lg border bg-muted/20 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><BrokerTitle broker={broker} /><p className="mt-1 text-sm text-muted-foreground">Для каждого счёта укажите номер и добавьте его отчёты отдельно.</p></div><Button variant="outline" onClick={onAddGroup}><Plus data-icon="inline-start" />Добавить счёт</Button></div><div className="mt-4 flex flex-col gap-4">{groups.length === 0 && <p className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">Счета не добавлены.</p>}{groups.map((group, index) => { const locked = group.files.some((report) => report.uploaded); return <div key={group.id} className="rounded-md border bg-card p-4"><div className="flex flex-wrap items-end gap-3"><label className="min-w-52 flex-1 text-sm font-medium">Номер счёта {groups.length > 1 ? index + 1 : ''}<input className="mt-2 h-9 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-60" value={group.accountId} disabled={locked} onChange={(event) => onAccountChange(group.id, event.target.value)} placeholder="Например, 759023" /></label><FilePicker className="min-w-64 flex-1" broker={broker} onFiles={(files) => onFiles(group.id, broker, files)} /><Button variant="ghost" size="icon" disabled={locked} onClick={() => onRemoveGroup(group.id)} aria-label="Удалить счёт" title={locked ? 'Принятый backend счёт можно удалить только вместе со всем расчётом' : 'Удалить счёт'}><Trash2 /></Button></div>{!group.accountId.trim() && group.files.length > 0 && <p className="mt-2 text-xs text-destructive">Укажите номер счёта Freedom.</p>}<ReportList reports={group.files} onRemove={(reportId) => onRemoveFile(group.id, reportId)} /></div>})}</div>{broker.code === 'freedom' && <div className="mt-4 flex flex-wrap items-center gap-x-2 gap-y-1 border-t border-primary/10 pt-3 text-sm"><BookOpen className="size-4 text-primary" aria-hidden="true" /><span className="font-medium">Инструкция:</span><Link href="/faq/freedom-broker" className="text-primary underline-offset-4 hover:underline">как скачать отчёты Freedom Broker</Link></div>}</div>
}
