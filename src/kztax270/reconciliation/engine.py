...
def _canonical_fifo_pnl_after_all_commissions_by_instrument(dataset: CanonicalDataset) -> dict[str, Decimal]:
    ...
    for record in dataset.tables.get("Fifo", []):
        # A fractional disposal generated by a stock split is part of the
        # instrument's realized FIFO P/L.  Freedom may fold it into the P/L
        # shown for the later ordinary sale, but reconciliation must still
        # include both FIFO exits in the canonical calculation.
        is_split_compensation = str(record.get("corporate_action_type") or "").casefold() == "split_compensation"
        if str(record.get("source_trade_id") or "").startswith("CA:") and not is_split_compensation:
            continue
